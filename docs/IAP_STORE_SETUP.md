# In-app purchase setup（30 次免费 → 一次性解锁）

应用内 **每成功生成一次字帖** 计 1 次；前 **30 次免费**，之后需购买 **非消耗型** 内购 `handwriting_practice_unlock` 解锁无限次。
...
| **Android** | 卸载会清除应用数据，**次数会重置**；已购「解锁」在 **安装后首次打开时自动恢复** |
...
审核备注可写：前 30 次生成字帖免费，第 31 次起弹出购买页；**安装后首次打开时自动恢复**已购解锁（换机 / 重装无需手动操作）。
...
- 提供 **恢复购买**能力（安装后首次打开时静默恢复，满足审核要求）
- App Store 审核说明中写清：30 次免费试用 + 一次性解锁
